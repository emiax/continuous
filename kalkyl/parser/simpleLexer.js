define(['quack', './exports.js' , 'kalkyl/package'], function(q, SP, KL) {
    return SP.SimpleLexer = q.createClass({
        
        constructor: function (str) {
            this._tokenBuffer = [];
            this._lastToken = null;
            var lexer = this._lexer = new SP.Lexer(str);
        ...
        addToBuffer: function (token) {
            ...
            if (token.type() === 'name') {
                ...
                strings.forEach(function (s) {
                    scope._tokenBuffer.push(new SP.Token('name', s, pos));
                    pos++;
                });
            } else {
        ...
        implicitMultiplication: function() {
            ...
            if (token) {
                ...
                case 'name':               
                    ...
                    case 'number':                        
                    case 'name':
                        this._tokenBuffer.unshift(new SP.Token('implicitMultiplication', '', this._lastToken.position()));
                    ...
                    case 'number':
                        ...
                        case 'ln': case 'lg': case 'sqrt':
                            this._tokenBuffer.unshift(
                                new SP.Token("implicitComma", token.string(), token.position())
                            );
                        }
                    }
